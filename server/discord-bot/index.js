...
const PRODUCT_FEEDBACK_CHANNEL = process.env.FEEDBACK_CHANNEL
...
const createFeedbackBot = (cohereClient) => new Promise((resolve) => {
  const client = new Client({ intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MESSAGES] })

  const classifyMessage = async (message) => {
    const response = await cohereClient.classify(process.env.FEEDBACK_MODEL_ID, { 
      inputs: [message] 
    }) 
  ...
  client.on('messageCreate', async msg => {
    ...
    if (FEEDBACK_CHANNEL_IDS.indexOf(channelId) !== -1) {
      ...
      if (confidence > 0.8) {
        ...
        const message = `${!!tag ? `<@${tag}> ` : ''}${author.username} just gave some product feedback for ${prediction} on Discord:\n\n${content}`
        await slack.client.chat.postMessage({ channel: PRODUCT_FEEDBACK_CHANNEL, text: message })
